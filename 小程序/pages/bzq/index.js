...
Page({
 ...
 2. 生命周期函数--监听页面加载
  ...
  btnGetDays:function(){
    ...
    let hydstdq = bzqtime - days;
    //判断是否是正数，如果是负数则返回失败
    if(days < 0){
      wx.showToast({
        title:'输入错误',
        image:'../../images/error.png', //这是要用到的“错误”图片
        duration:2000
      })
    } else{
      that.setData({
  ...
  bindbzqinput:function(e){
    this.setData({
        bzqtime: parseInt(e.detail.value)
      })
},
  ...
  bindstar:function(){
    var shiyong = this.data.days
    var d = this.data.bzqtime
    var hydst = d - shiyong
    console.log(shiyong);
    this.setData({
        hydst:hydst,
